...
Function Test-CommandExist() {
    <#
     ...
     .DESCRIPTION
       use Get-Command to test command exist

     .PARAMETER CommandName
       command name

     .PARAMETER NotExistExit
       if not exist exit

     .PARAMETER NotExitCode
       exit code
     ...
     .EXAMPLE
       Test-CommandExist foo-commd
       Test-CommandExist foo-commd -Exit
       Test-CommandExist -CommandName foo-commd -NotExistExit
       Test-CommandExist -CommandName foo-commd -NotExistExit -NotExitCode 2
     #>
    [OutputType([Boolean])]
    param
    (
        [Parameter(
            Mandatory = $False,
            HelpMessage = "Parameter missing: -CommandName as string")]
        [string] $CommandName,
        [Alias('Exit')]
        [switch]$NotExistExit,
        [int]$NotExitCode = 1
    ...
    $out = $false
    try {
        $out = ((Get-Command $CommandName -ErrorAction Stop).length -gt 0)
    }
    catch {
        # do nothing
        if ($NotExistExit) {
